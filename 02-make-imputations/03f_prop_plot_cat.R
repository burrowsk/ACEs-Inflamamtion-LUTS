...
propplot_cat <- function(x, formula, facet = "wrap", ...) {
  library(ggplot2)
  ...
  p <- ggplot(plotDF, aes(x = value, fill = get(svars), y = prop)) +
    ...
    guides(fill = guide_legend(nrow = 1, title = "Imputation Number")) +
    theme(strip.text = element_text(size = 8), text = element_text(size = 10))
