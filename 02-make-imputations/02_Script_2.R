...
#===============================================================================
# This is an adaptation of Script_2.R created by Lotte Houtepen
# Kimberley Burrows
# Completed:
...
loc_inp='C:/Users/epwkb/OneDrive - University of Bristol/MHI/02-Inflammation-mediation/02-analysis/data/' # input files
loc_out='C:/Users/epwkb/OneDrive - University of Bristol/MHI/02-Inflammation-mediation/02-analysis/data/' # output files
setwd(loc_inp)
setwd(loc_out)
...
threshold_proportion_answered_ACE0_18<-0.1
...
sample_Boyd<-FALSE # Change to FALSE if you want to keep n=15455
...
dim(binary_alspacKids_ACE_data) # 13739 observations for 835 variables
...
which(adv_description$time_yrs1>=0 &
        adv_description$time_yrs2<=8) # I have changed this to my specified age
]))
...
binary_alspacKids_ACE_data<-
  binary_alspacKids_ACE_data[
    !(rowSums(is.na(binary_alspacKids_ACE_data[,ACE_var0_18]))/
        length(ACE_var0_18))>=(1-threshold_proportion_answered_ACE0_18),]
# There are 11653 individuals with >10% of ACE variables
...
prop.table(table(ACE_time1_time2$bullying, useNA="always"))

prop.table(table(ACE_time1_time2$physical_abuse))
prop.table(table(ACE_time1_time2$sexual_abuse))
prop.table(table(ACE_time1_time2$emotional_abuse))
prop.table(table(ACE_time1_time2$emotional_neglect))
prop.table(table(ACE_time1_time2$violence_between_parents))
prop.table(table(ACE_time1_time2$substance_household))
prop.table(table(ACE_time1_time2$mental_health_problems_or_suicide))
prop.table(table(ACE_time1_time2$parent_convicted_offence))
prop.table(table(ACE_time1_time2$parental_separation))
prop.table(table(ACE_time1_time2$bullying))
...
ncol(alspacKids_ACE_data_2018) # Should include 253 variables // has 247 (4 vars no longer available)
...
# also saved as .dta for stata // note that var names are shortened as per 31 chara limit in stata
