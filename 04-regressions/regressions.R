...
#===============================================================================
# Kimberley Burrows
# Regressions for Table 1, 2 and S8 using imputed data
#===============================================================================
...
# Notes: -----------------------------------------------------------------------
# Paper Table 1: LUTS ~ ACE score [+ confs]
# Paper Table 2: LUTS ~ Inflammation [+ conf + ACE score + L's]
# Paper Table S8: Inflammation ~ ACEs [+ confs] & Inflammation ~ Inflammation [+ confs + ACE score + L's]
...
for (i in seq_along(vars)) {
  mods[[j]][[i]] <- with(imp_use,
                    glm(as.formula(paste0(outcomes[[j]], " ~ ", vars[[i]])), family = binomial(link = "logit"))) %>%
    tbl_regression(
      exponentiate = TRUE,
...
gtsave(table, file = paste0(Tables,"table_1.docx"))
rm(table, results, mods)
gc()
#===============================================================================

#===============================================================================
## TABLE 2. LUTS ~ Inflammation [+ conf + ACE score + L's]
conf2 <- c("Sex",
          "Birthweight",
          "Gestational_age",
          "Ethnicity",
...
gtsave(table, file = paste0(Tables,"table_2.docx"))
...
#===============================================================================
## TABLE S8. Inflammation ~ ACEs [+ confs]
conf <- c("Sex",
          "Birthweight",
          "Gestational_age",
          "Ethnicity",
...
for (j in seq_along(outcomes)) {
  ...
  for (i in seq_along(vars)) {
    mods[[j]][[i]] <- with(imp_use,
                           glm(as.formula(paste0(outcomes[[j]], " ~ ", vars[[i]])), family = gaussian())) %>%
      tbl_regression(
        exponentiate = TRUE,
        show_single_row = ACE_score,
        label = list(ACE_score ~ outcomes[[j]])
...
gtsave(table, file = paste0(Tables,"table_s8.docx"))
